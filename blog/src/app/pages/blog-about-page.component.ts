...
import { BLOG_SEO_SERVICE, type SocialPlatform } from '@foliokit/cms-core';
import { ThemeService } from '@foliokit/cms-ui';

/**
 * Same behavior as FolioKit {@link BlogAboutPageComponent}, but imports
 * standalone {@link MarkdownComponent} instead of {@link MarkdownModule}.
 * The NgModule + root `provideMarkdown()` combo on a lazy route triggers
 * NG0203 / NG0200 (`StandaloneService` / `EnvironmentInjector`) in Angular 21.
 */
...
export class BlogAboutPageComponent {
  ...
  protected readonly about = toSignal(
    this.route.data.pipe(map((data) => data['about'] ?? null)),
    { initialValue: this.route.snapshot.data['about'] ?? null },
  );
  ...
  protected platformIcon(platform: SocialPlatform | undefined): string {
